...
"""Check that the distribution and danceamole/__init__.py are in sync."""
import os
import pathlib
import subprocess
import sys
...
def main() -> int:
    ...
    if setup_py_map["version"] != danceamole.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in danceamole/__init__.py is: "
            f"{danceamole.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != danceamole.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in danceamole/__init__.py is: "
            f"{danceamole.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != danceamole.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in danceamole/__init__.py is: "
            f"{danceamole.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != danceamole.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in danceamole/__init__.py is: "
            f"{danceamole.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != danceamole.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in danceamole/__init__.py, "
                f"but found: {danceamole.__status__}"
            )
            success = False
